...
def get_user_difficulty() -> float:
    print("\nWYBIERZ POZIOM ZAGĘSZCZENIA PRZESZKÓD:")
    print("1. Mało (5% mapy)")
...
def main() -> None:
    print(f"=== SYSTEM SYMULACJI BSP (Wersja z Legendą po Lewej) ===")
    ...
    print_results("0. Dijkstra", stats_d)
    if path_d:
        # Przekazujemy stats_d i nazwę osobno
        plot_simulation(env, path_d, stats_d, "Algorytm Dijkstry", block=False)

    # === SCENARIUSZ 1: A* STANDARD ===
    path_a, stats_a = run_astar(env, start_pos, goal_pos)
    print_results("1. A* Standard", stats_a)
    if path_a:
        plot_simulation(env, path_a, stats_a, "A* Standard", block=False)

    # === SCENARIUSZ 2: RISK-AWARE A* (Statyczny) ===
    path_risk, stats_risk = run_risk_astar(env, start_pos, goal_pos, risk_weight=20.0)
    print_results("2. Risk-Aware A* (W=20)", stats_risk)
    # Tu opcjonalnie też można wyświetlić
